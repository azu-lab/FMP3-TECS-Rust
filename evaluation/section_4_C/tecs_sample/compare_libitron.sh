...
# Helper: sum per-ex totals from libitron_objects.csv
sum_totals() {
  local csv_dir="$1"  # directory containing ex*/libitron_objects.csv
  local out_csv="$2"
  printf "ex,text_dec,rodata_dec,data_dec,bss_dec,other_dec,total_dec\n" > "$out_csv"
  shopt -s nullglob
  for f in "$csv_dir"/ex*/libitron_objects.csv; do
    local dir
...
TR_TOT_NO="$OUT_DIR/libitron_totals_tr_noopt.csv"
TR_TOT_OP="$OUT_DIR/libitron_totals_tr_opt.csv"
R_TOT="$OUT_DIR/libitron_totals_r.csv"
...
THREE_TOT="$OUT_DIR/libitron_three_totals_all.csv"
...
if [[ -d "$R_BASE_DIR" ]]; then
  BCSV="$R_BASE_DIR/libitron_objects.csv"
  for d in "$R_ROOT"/ex*; do
    [[ -d "$d" ]] || continue
    DCSV="$d/libitron_objects.csv"
    [[ -f "$BCSV" && -f "$DCSV" ]] || continue
    awk -F, 'NR==FNR{ if(NR>1) base[$2]=$8; next } NR>1 { if ($8 != base[$2]) print }' "$BCSV" "$DCSV" >> "$OUT_DIR/libitron_unique_r.csv"
  done
fi

echo "Wrote: $OUT_DIR/libitron_unique_tr.csv"
echo "Wrote: $OUT_DIR/libitron_unique_r.csv"

# Build per-ex combined unique summary (R vs TR_no_opt vs TR_opt)
U3="$OUT_DIR/libitron_unique_three_all.csv"
printf "ex,r_object,r_total,tr_no_object,tr_no_total,tr_opt_object,tr_opt_total\n" > "$U3"
for n in 1 2 3 4 5; do
  exr="ex${n}"
  exno="ex${n}_no_opt"
  exop="ex${n}_opt"
  r_line=$(grep -m1 "^${exr}," "$OUT_DIR/libitron_unique_r.csv" || true)
  no_line=$(grep -m1 "^${exno}," "$OUT_DIR/libitron_unique_tr.csv" || true)
  op_line=$(grep -m1 "^${exop}," "$OUT_DIR/libitron_unique_tr.csv" || true)
