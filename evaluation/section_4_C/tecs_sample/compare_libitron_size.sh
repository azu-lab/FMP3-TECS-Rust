...
# Helper: sum per-ex totals from libitron_size_objects.csv
sum_totals() {
  local csv_dir="$1"
  local out_csv="$2"
  printf "ex,text_dec,rodata_dec,data_dec,bss_dec,other_dec,total_dec\n" > "$out_csv"
  shopt -s nullglob
  for f in "$csv_dir"/ex*/libitron_size_objects.csv; do
    local dir
...
TR_TOT_NO="$OUT_DIR/libitron_size_totals_tr_noopt.csv"
TR_TOT_OP="$OUT_DIR/libitron_size_totals_tr_opt.csv"
R_TOT="$OUT_DIR/libitron_size_totals_r.csv"
...
THREE_TOT="$OUT_DIR/libitron_size_three_totals_all.csv"
...
# Unique object detection per tree (size-based, baseline diff)
printf "dir,object,text_dec,rodata_dec,data_dec,bss_dec,other_dec,total_dec\n" > "$OUT_DIR/libitron_size_unique_tr.csv"
printf "dir,object,text_dec,rodata_dec,data_dec,bss_dec,other_dec,total_dec\n" > "$OUT_DIR/libitron_size_unique_r.csv"

TR_BASE="$(ls -d "$TR_ROOT"/ex*_no_opt 2>/dev/null | head -n1)"
if [[ -n "$TR_BASE" ]]; then
  BCSV="$TR_BASE/libitron_size_objects.csv"
  for d in "$TR_ROOT"/ex*_no_opt "$TR_ROOT"/ex*_opt; do
    [[ -d "$d" ]] || continue
    DCSV="$d/libitron_size_objects.csv"
    [[ -f "$BCSV" && -f "$DCSV" ]] || continue
    awk -F, 'NR==FNR{ if(NR>1) base[$2]=$8; next } NR>1 { if ($8 != base[$2]) print }' "$BCSV" "$DCSV" >> "$OUT_DIR/libitron_size_unique_tr.csv"
  done
fi

R_BASE_DIR="/home/yosshy/research/FMP3-TECS/r_opt_sample/ex1"
if [[ -d "$R_BASE_DIR" ]]; then
  BCSV="$R_BASE_DIR/libitron_size_objects.csv"
  for d in "$R_ROOT"/ex*; do
    [[ -d "$d" ]] || continue
    DCSV="$d/libitron_size_objects.csv"
    [[ -f "$BCSV" && -f "$DCSV" ]] || continue
    awk -F, 'NR==FNR{ if(NR>1) base[$2]=$8; next } NR>1 { if ($8 != base[$2]) print }' "$BCSV" "$DCSV" >> "$OUT_DIR/libitron_size_unique_r.csv"
  done
fi

echo "Wrote: $OUT_DIR/libitron_size_unique_tr.csv"
echo "Wrote: $OUT_DIR/libitron_size_unique_r.csv"

# Build per-ex combined unique summary (R vs TR_no_opt vs TR_opt)
U3="$OUT_DIR/libitron_size_unique_three_all.csv"
printf "ex,r_object,r_total,tr_no_object,tr_no_total,tr_opt_object,tr_opt_total\n" > "$U3"
for n in 1 2 3 4 5; do
  exr="ex${n}"
  exno="ex${n}_no_opt"
  exop="ex${n}_opt"
  r_line=$(grep -m1 "^${exr}," "$OUT_DIR/libitron_size_unique_r.csv" || true)
  no_line=$(grep -m1 "^${exno}," "$OUT_DIR/libitron_size_unique_tr.csv" || true)
  op_line=$(grep -m1 "^${exop}," "$OUT_DIR/libitron_size_unique_tr.csv" || true)
