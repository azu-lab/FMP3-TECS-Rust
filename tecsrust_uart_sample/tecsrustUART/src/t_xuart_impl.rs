...
impl SiHandlerBody for EiHandlerBodyForTXuart<'_>{
	...
	fn main(&'static self) {
		...
		// 受信
		unsafe{
			// if (read_volatile((*lg.base_address + XUART_ISR_OFFSET) as *const u32) & XUART_IXR_RXTRIG) == 1 {
			// 	while (read_volatile((*lg.base_address + XUART_ISR_OFFSET) as *const u32) & XUART_SR_RXEMPTY) == 0 {
			// 		c_xuart_main.ready_receive();
			// 	}
			// }

			if (read_volatile((*lg.base_address + XUART_SR_OFFSET) as *const u32) & XUART_SR_RXEMPTY) == 0 {
				lg.c_xuart_main.ready_receive();
			}
		...
		// 送信
		unsafe{
			// if (read_volatile((*lg.base_address + XUART_ISR_OFFSET) as *const u32) & XUART_IXR_TXEMPTY) == 1 {
			// 	while (read_volatile((*lg.base_address + XUART_ISR_OFFSET) as *const u32) & XUART_SR_TXFULL) == 0 {
			// 		c_xuart_main.ready_send();
			// 	}
			// }

			if (read_volatile((*lg.base_address + XUART_SR_OFFSET) as *const u32) & XUART_SR_TXFULL) == 0 {
				lg.c_xuart_main.ready_send();
			}
