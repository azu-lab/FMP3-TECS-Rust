/*
 * This file was automatically generated by tecsgen.
 * This file is not intended to be edited.
 */
#include "tLogTaskMain_tecsgen.h"
#include "tLogTaskMain_factory.h"

/* entry port descriptor type #_EDT_# */
/* eLogTaskBody */
struct tag_tLogTaskMain_eLogTaskBody_DES {
    const struct tag_sTaskBody_VMT *vmt;
    tLogTaskMain_IDX  idx;
};

/* eLogTaskTerminate */
struct tag_tLogTaskMain_eLogTaskTerminate_DES {
    const struct tag_sRoutineBody_VMT *vmt;
    tLogTaskMain_IDX  idx;
};

/* eLogTask : omitted by entry port optimize */

/* entry port skelton function #_EPSF_# */
/* eLogTaskBody */
void           tLogTaskMain_eLogTaskBody_main_skel( const struct tag_sTaskBody_VDES *epd)
{
    tLogTaskMain_eLogTaskBody_main( );
}
/* eLogTaskTerminate */
void           tLogTaskMain_eLogTaskTerminate_main_skel( const struct tag_sRoutineBody_VDES *epd)
{
    tLogTaskMain_eLogTaskTerminate_main( );
}
/* eLogTask : omitted by entry port optimize */

/* entry port skelton function table #_EPSFT_# */
/* eLogTaskBody */
const struct tag_sTaskBody_VMT tLogTaskMain_eLogTaskBody_MT_ = {
    tLogTaskMain_eLogTaskBody_main_skel,
};
/* eLogTaskTerminate */
const struct tag_sRoutineBody_VMT tLogTaskMain_eLogTaskTerminate_MT_ = {
    tLogTaskMain_eLogTaskTerminate_main_skel,
};
/* eLogTask : omitted by entry port optimize */

/* entry port descriptor referenced by call port (differ from actual definition) #_CPEPD_# */

/* call port array #_CPA_# */

/* array of attr/var #_AVAI_# */
/* cell INIB #_INIB_# */
tLogTaskMain_INIB tLogTaskMain_SINGLE_CELL_INIB = 
{
    /* entry port #_EP_# */ 
    /* attribute(RO) */ 
    10000,                                   /* interval */
    1000,                                    /* flushWait */
};

extern const struct tag_sTaskBody_VMT tLogTaskMain_eLogTaskBody_MT_;
extern const struct tag_sRoutineBody_VMT tLogTaskMain_eLogTaskTerminate_MT_;
/* entry port descriptor #_EPD_# */
extern const struct tag_tLogTaskMain_eLogTaskBody_DES rProcessor1Migratable_LogTask_LogTaskMain_eLogTaskBody_des;
const struct tag_tLogTaskMain_eLogTaskBody_DES rProcessor1Migratable_LogTask_LogTaskMain_eLogTaskBody_des = {
    &tLogTaskMain_eLogTaskBody_MT_,
    &tLogTaskMain_SINGLE_CELL_INIB,      /* INIB 3 */
};
extern const struct tag_tLogTaskMain_eLogTaskTerminate_DES rProcessor1Migratable_LogTask_LogTaskMain_eLogTaskTerminate_des;
const struct tag_tLogTaskMain_eLogTaskTerminate_DES rProcessor1Migratable_LogTask_LogTaskMain_eLogTaskTerminate_des = {
    &tLogTaskMain_eLogTaskTerminate_MT_,
    &tLogTaskMain_SINGLE_CELL_INIB,      /* INIB 3 */
};
/* eLogTask : omitted by entry port optimize */
/* CB initialize code #_CIC_# */
void
tLogTaskMain_CB_initialize()
{
    SET_CB_INIB_POINTER(i,p_cb)
    INITIALIZE_CB()
}
